...
$Gini(t)=1-\sum_{j=1}^{C}(p(j\vertt))^2$

$p(j\vertt):probability~of~class~j~to~occur~on~node~t$
...
$Entropy(t) = -\sum_{j}^{C}p(j\vertt)log(p(j\vertt))$
...
$Error(t) = 1-max_{j\in C}(p(j\vertt))$
...
- How to construct an ROC curve?
    1. Use classifier that produces posterior probability for each test instance P(+\vertA)
    2. Sort the instances according to P(+\vertA) in decreasing order
    3. Apply threshold at each unique value of P(+\vertA)
