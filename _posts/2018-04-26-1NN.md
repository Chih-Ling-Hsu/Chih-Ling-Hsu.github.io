...
$$
\|\overrightarrow{y_{min}}-\overrightarrow{x}\| = {min}_{1\leq i \leq 128} \|\overrightarrow{y_i}-\overrightarrow{x}\| = {min}_{1\leq i \leq 128} \|\sum_{j=1}^{16}(y_{ij}-x_j)^2\|
$$


Assume some centers (e.g., $\overrightarrow{y_1}, \overrightarrow{y_2}, ...,$ represented as $\overrightarrow{y_l}$) have been examined so far and the smallest minimum squared error is
...
Here I will introduce 5 methods for finding the nearest center:
...
This is because if $\|\overrightarrow{y_i} -\overrightarrow{y_{min}^{SF}}\| \geq 2 d_{min}$ then
...
$$
(Sum(\overrightarrow{x})-Sum(\overrightarrow{y_i}))^2 \leq 16 \|\overrightarrow{x}-\overrightarrow{y_i}\|^2
$$
...
\\
\geq 4\cdot {d_{min}^2}^{SF} = 4\cdot \|\overrightarrow{x}-\overrightarrow{y_i^{SF}}\|^2
$$
...
Sort {$\overrightarrow{y_1}, ..., \overrightarrow{y_{128}}$} according to their absolute values in ascending order and evaluate $2\|\overrightarrow{x}\|$.
...
$$
d^2(\overrightarrow{x}, \overrightarrow{y_i}) < (\tilde{d}_{min}^{SF})^2
$$
...
| $\because \|\overrightarrow{y_i}-\overrightarrow{y_j}\|$ | $\because \|\overrightarrow{y_i}\|_{l2}, \|\overrightarrow{y_i}\|_{MAX}, \|\overrightarrow{y_i}\|_{l1}$ | $\because Sum(\overrightarrow{y_i}), H(\overrightarrow{y_i}), V(\overrightarrow{y_i})$ | $\because \|\overrightarrow{y_i}\|$| 
